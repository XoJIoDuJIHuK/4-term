...
HANDLE hAcceptServer, hConsolePipe, hGarbageCleaner, hDispatchServer, hResponseServer, hTimerServer;
...
void CALLBACK SetServiceTimer(ULONG_PTR p);
			...
			for (auto i = contacts.begin(); i != contacts.end(); i++) {
				...
				if (i->type == i->ACCEPT) {
					...
					if (strcmp(i->msg, "Echo") && strcmp(i->msg, "Time") && strcmp(i->msg, "Random")) {
						...
						QueueUserAPC(SetServiceTimer, hTimerServer, (ULONG_PTR)&*i);
					}
				}
			}
			Sleep(200);
...
bool PutAnswerToClient(const char* msg, sockaddr* to, int* lto) {
...
bool GetRequestFromClient(char* name, short port, SOCKADDR_IN* from, int* flen)
{
	...
	*flen = lc;
	...
	puts("\nbad name");
	PutAnswerToClient("bad name", (sockaddr*)&clnt, &lc);
	return false;
	...
	while (*(TalkersCommand*)pPrm != EXIT)
	{
		try
		{
			if (GetRequestFromClient("Hello", serverPort, &from, &lc))
			{
				...
				PutAnswerToClient(inet_ntoa(serv.sin_addr), (sockaddr*)&from, &lc);
			}
		}
		catch (string errorMsgText)
		{
...
//
DWORD WINAPI TimerServer(LPVOID pPrm)
{
	DWORD rc = 0;
	const TalkersCommand& COMMAND = *(TalkersCommand*)pPrm;
	while (COMMAND != TalkersCommand::EXIT) {
		SleepEx(0, TRUE);
	}
	ExitThread(rc);
}
...
void CALLBACK SetServiceTimer(ULONG_PTR p)
{
	...
	if (!(contact.htimer = CreateWaitableTimer(nullptr, FALSE, nullptr)))
		throw SetErrorMsgText("CreateWaitableTimer:", WSAGetLastError());
	LARGE_INTEGER li{0, 0};
	li.QuadPart = -10000000LL * 5;
	cout << li.QuadPart << endl;
	if (!SetWaitableTimer(contact.htimer, &li, 0, ASWTimer, &contact, FALSE))
		throw SetErrorMsgText("SetWaitableTimer:", WSAGetLastError());
}
...
int main(int argc, char* argv[])
{
	...
	hTimerServer = CreateThread(
		nullptr, NULL, TimerServer, (LPVOID)&cmd, NULL, nullptr);
	//...
	WaitForSingleObject(hTimerServer, INFINITE);
	CloseHandle(hTimerServer);
