...
//---------------------------------------------------------------------------------
CMatrix::CMatrix(int Nrow = 3, bool isSpheric = false)  //Вектор
// Nrow - число строк
{
	n_rows = 3;
	n_cols = 1;
	array = new double*[n_rows];
	for (int i = 0; i < n_rows; i++) array[i] = new double[n_cols];
	array[0][0] = rand() % 50 - 40;
	double value = rand() % (int)pow(10, 2);

	// получить вещественное число
	value = (value / pow(10, 2)) * (M_PI);
	array[1][0] = value;
	value = rand() % (int)pow(10, 2);
	value = (value / pow(10, 2))* (M_PI * 2);
	array[2][0] = value;
}
//---------------------------------------------------------------------------------
CMatrix::~CMatrix()
